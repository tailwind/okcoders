...
class SearchListingItem extends Component {
	constructor(props) {
		super(props);
		const viralityScore = _.floor(
			this.props.data.repin / this.props.data.pin_count
		);
		...
		return (
			<div className="animated fadeInUp">
				<ListGroupItem key={this.state.board.board_id}>
					<h1>
						<Image
							src={this.state.board.board_cover_image_url}
						...
						<OverlayTrigger
							placement="bottom"
							overlay={
								<Tooltip id="tooltip">{this.state.board.description}</Tooltip>
							}
						>
					...
					{!(this.state.board.category === "NULL") && (
						<Link to={`/category/${this.state.board.category}`}>
							<h4>{this.state.prettyCategoryName}</h4>
						</Link>
					)}
					...
					<h4>
						...
						<OverlayTrigger
							...
							overlay={
								<Tooltip id="tooltip">
									We calculated this number by dividing the repins by total
									pins.
								</Tooltip>
							}
						>
					...
					<a href={`http://www.pinterest.com/${this.state.board.username}`}>
						<Image src={this.state.board.image} responsive circle />
					</a>
					<h4>
						Admin:{" "}
						<a href={`http://www.pinterest.com/${this.state.board.username}`}>
					...
					{!(this.state.board.website_url === "NULL") && (
						<h4>
							Admin Site:{" "}
							<a href={`${this.state.board.website_url}`}>
								{this.state.board.website_url}
							</a>
						</h4>
					)}
					...
					<Modal show={this.state.showModal} onHide={this.closeModal}>
						<Modal.Header closeButton>
							<Modal.Title>
								Request to join {this.state.board.board_name} as a collaborator:{" "}
							</Modal.Title>
						</Modal.Header>
						<Modal.Body>
							<form>
								...
								<FormGroup controlId="formControlsTextarea">
									<ControlLabel>
										Enter a message for the Group Board owner:
									</ControlLabel>
									<FormControl
										componentClass="textarea"
										placeholder="500 characters available"
										maxLength="500"
									/>
								</FormGroup>
							</form>
